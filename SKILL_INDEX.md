# Skill Index

## Upstream Agents

- [`amazon-product-manager-skill`](https://github.com/ianyoufather-2007/amazon-product-manager-skill) — 核心 Amazon PM 判断框架。
- [`01-product-manager-workflow-agent`](https://github.com/ianyoufather-2007/01-product-manager-workflow-agent) — PM 判断、机会初筛、风险梳理、老板汇报。
- 000-007 stage-gate workflow package — 本地/私有漏斗执行包；本仓库只发布干净的路由层、阶段门摘要和脱敏示例。

## Stage-Gate Key Files

- `AGENT.md` — 公开 agent 入口和路由原则。
- `docs/workflow-routing.md` — 输入资料和阶段路线的对应关系。
- `docs/stage-gates.md` — 000-007 阶段门摘要。
- `templates/` — 000-007 可复制 prompt 模板。
- `examples/` — 脱敏输入和预期输出示例。

## Local Docs

- `docs/workflow-routing.md` — 01/02/03 路由关系。
- `docs/stage-gates.md` — 研究阶段门摘要。
- `docs/open-source-cleanup-checklist.md` — 后续开源清理清单。

## Examples

- `examples/quick-opportunity-triage.md` — 资料不足时先做 PM 初判。
- `examples/stage-run.md` — 有数据时进入研究阶段执行。
- `examples/anonymized-opportunity-review.md` — 脱敏机会评审完整输出示例。
- `examples/anonymized-low-data-triage.md` — 只有产品想法和零散观察时的 PM Triage 示例。
- `examples/anonymized-bsr-asin-stage-run.md` — 已有 BSR/关键词/ASIN 时进入阶段 001-004 的示例。

## Templates

- `templates/stage-000-triage.md` — 机会快筛和最小数据清单。
- `templates/stage-001-company-fit.md` — 公司能力、预算、供应链和运营匹配度。
- `templates/stage-002-demand-trend.md` — 需求强度、趋势和季节性判读。
- `templates/stage-003-market-structure.md` — 市场结构、价格带、评分/评论壁垒。
- `templates/stage-004-competitor-map.md` — 竞品池清洗、功能矩阵和差异化假设。
- `templates/stage-005-voc-definition.md` — VOC、使用场景、痛点和产品定义假设。
- `templates/stage-006-roi-risk.md` — ROI 区间、成本压力、合规/专利/供应链风险。
- `templates/stage-007-validation.md` — 样品、QC、包装、用户反馈和会议决策支持。
